...
PlotTimeRaster <- function(step = "month"){
  ...
  points <- points[order(points@data[,1], decreasing=TRUE),]
  points <- points[1:10,]
  points <- spTransform(points, CRS=(wgs84))
  names(points) <- "potential"
