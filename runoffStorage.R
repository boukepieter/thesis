...
testplot.storages <- function(res, debiet, poi, factors){
  nul <- extract(debiet,poi)
  facts <- lapply(res,FUN=extract,y=poi)
  plot(nul[1,],col="red", type='b', xlab="month", ylab="runoff (m3/s)", xlim=c(1,nlayers(debiet)))
  cols <- c("blue", "green")
  for(i in 2:length(facts)){
    lines(facts[[i]][1,], col=cols[i-1], type='b')
    print(i)
  }
  lines(rep(0,12), col="black", type='l', lty=2)
  legend("topleft", legend=c("0",rev(factors)), col=c("red",cols), pch=1)
  title(paste(sprintf("Runoff at point %d, %d for factors", 
                      as.integer(poi@coords)[1], as.integer(poi@coords)[2]), 
              do.call(paste, c("0",factors,sep=", "))))
}
